...
static ssize_t gamepad_read(struct file *, char *, size_t, loff_t *);        // user program reads from the driver.
static ssize_t gamepad_write(struct file *, const char *, size_t, loff_t *); // user program writes to the driver.
static int gamepad_open(struct inode *, struct file *);                      // user program opens the driver.
static int gamepad_release(struct inode *, struct file *);                   // user program closes the driver.
...
static int __init gamepad_init(void)
{
    ...
    printk(KERN_INFO "gamepad: major number allocated successful\n");
    ...
    cdev_init(&character_device, &fops); // The new way to initialize a character device driver, according to the compendium.
    int error_code = cdev_add(&character_device, device_number, 1);

    if (error_code < 0) {
        printk(KERN_ALERT "gamepad: registering character device failed with %d\n", error_code);
        return error_code;
    }

    // Mounts the gamepad driver as /dev/gamepad.
...
// Called when a process tries to open the device file, e.g cat /dev/mycharfile.
...
static ssize_t gamepad_read(struct file *filp, char *buffer, size_t count, loff_t * offset)
{
    ...
    *offset += 4;
	...
	return 4; // returns the number of bytes read.
}
 
// Called when a process writes to dev file e.g echo "hi" > /dev/hello 
